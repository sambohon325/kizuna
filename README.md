...
- Cooldown-deduplicated external operations alerts through SMTP and signed generic webhooks, with safe delivery history and manual test delivery
...
1. Run an isolated full-stack Coolify recovery rehearsal and record its evidence
2. Design guarded project import that never overwrites active work without an explicit recovery decision
3. Add structured-log retention/export policy and operational recovery notifications
...
See [external operations alerts](docs/OPERATIONS_ALERTS.md) for email, signed webhook, cooldown, retry, and verification setup.
