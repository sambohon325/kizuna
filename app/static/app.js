...
async function setupStorageConsole(projectId) {
  ...
  try {
    const [policy,schedule,storage,media,backups,links,studio]=await Promise.all([api(`/api/projects/${projectId}/storage-policy`),api(`/api/projects/${projectId}/backup-schedule`),api('/api/settings/storage'),api(`/api/projects/${projectId}/media-index`),api(`/api/projects/${projectId}/backups`),api(`/api/projects/${projectId}/delivery-links`),api(`/api/projects/${projectId}/compositor`)]);
    ...
    host.querySelector('.storage-grid').insertAdjacentHTML('beforebegin',renderMediaFootprint(media,storage));
...
let integrationSettings=null,aiRoutingSettings=null,computeSettings=null,storageSettings=null,settingsView='compute';
...
async function openSettings() {
  ...
  try{[integrationSettings,aiRoutingSettings,computeSettings,storageSettings]=await Promise.all([api('/api/settings/integrations'),api('/api/settings/ai-routing'),api('/api/settings/compute'),api('/api/settings/storage')]);renderIntegrationSettings();}catch(error){host.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;}
}
...
function organizeSettingsViews(host){
  host.insertAdjacentHTML('afterbegin',renderStorageSettings());const compute=document.createElement('div'),storage=document.createElement('div'),connections=document.createElement('div'),tabs=document.createElement('nav');compute.className=storage.className=connections.className='settings-pane';compute.dataset.settingsPane='compute';storage.dataset.settingsPane='storage';connections.dataset.settingsPane='connections';tabs.className='settings-tabs';tabs.innerHTML='<button type="button" data-settings-view="compute">Computers & costs</button><button type="button" data-settings-view="storage">Storage & backups</button><button type="button" data-settings-view="connections">AI & connected tools</button>';
  [...host.children].forEach(child=>(child.matches('.compute-control,.spend-monitor')?compute:child.matches('.storage-settings')?storage:connections).appendChild(child));host.replaceChildren(tabs,compute,storage,connections);const show=view=>{settingsView=view;compute.hidden=view!=='compute';storage.hidden=view!=='storage';connections.hidden=view!=='connections';tabs.querySelectorAll('button').forEach(button=>button.classList.toggle('active',button.dataset.settingsView===view));};tabs.querySelectorAll('button').forEach(button=>button.onclick=()=>show(button.dataset.settingsView));host.querySelector('#test-s3-storage')?.addEventListener('click',testS3Storage);show(settingsView);
}
...
function renderComputeControl(){
  ...
  const dayNames=['M','T','W','T','F','S','S'],taskNames={master_segment:'Video rendering',character_reference:'Character images'};
