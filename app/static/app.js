...
let integrationSettings=null,aiRoutingSettings=null,computeSettings=null,settingsView='compute';
...
async function openSettings() {
  ...
  try{[integrationSettings,aiRoutingSettings,computeSettings]=await Promise.all([api('/api/settings/integrations'),api('/api/settings/ai-routing'),api('/api/settings/compute')]);renderIntegrationSettings();}catch(error){host.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;}
}
...
function organizeSettingsViews(host){
  const compute=document.createElement('div'),connections=document.createElement('div'),tabs=document.createElement('nav');compute.className='settings-pane';connections.className='settings-pane';compute.dataset.settingsPane='compute';connections.dataset.settingsPane='connections';tabs.className='settings-tabs';tabs.innerHTML='<button type="button" data-settings-view="compute">Computers & costs</button><button type="button" data-settings-view="connections">AI & connected tools</button>';
  [...host.children].forEach(child=>(child.matches('.compute-control,.spend-monitor')?compute:connections).appendChild(child));host.replaceChildren(tabs,compute,connections);const show=view=>{settingsView=view;compute.hidden=view!=='compute';connections.hidden=view!=='connections';tabs.querySelectorAll('button').forEach(button=>button.classList.toggle('active',button.dataset.settingsView===view));};tabs.querySelectorAll('button').forEach(button=>button.onclick=()=>show(button.dataset.settingsView));show(settingsView);
}
...
function renderComputeControl(){
  ...
  const nodeCards=computeSettings.nodes.length?computeSettings.nodes.map(node=>`<article class="compute-node"><header><div><b>${safe(node.name)}</b><small>${safe(node.os_name)} · ${safe(node.architecture)}</small></div><span class="${safe(node.status)}">${safe(node.status)}</span></header><div class="node-specs"><span>${node.logical_cores} threads</span><span>${node.ram_gb} GB RAM</span><span>${node.gpu.length?safe(node.gpu.map(gpu=>gpu.name).join(', ')):'CPU only'}</span><span>score ${node.benchmark_score}</span></div><p>${node.strengths.length?`Best for ${safe(node.strengths.join(', '))}.`:'General support tasks.'}</p><details><summary>Detected creative tools · ${node.software.length}</summary><div class="software-list">${node.software.map(name=>`<span>${safe(name)}</span>`).join('')||'<span>None shared</span>'}</div></details></article>`).join(''):'<div class="compute-empty"><b>No local computers connected yet.</b><span>Cloud-only production still works. Add a computer when you want private AI, local previews, or rendering.</span></div>';
