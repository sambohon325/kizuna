...
function renderProductionFlow() {
  ...
  host.innerHTML=`<span class="flow-label"><b>${safe(project.title)}</b><small>${status.complete_count}/${status.total_count} complete</small></span><nav class="flow-track" aria-label="Production milestones">${productionStages.map((definition,index)=>{const stage=byKey.get(definition.key)||{state:'blocked',summary:'Status unavailable',label:definition.label},viewing=current===definition.key;return `<button type="button" class="flow-node ${safe(stage.state)} ${viewing?'viewing':''}" data-flow-stage="${safe(definition.key)}" title="Open ${safe(stage.label)} — ${safe(stage.summary)}" aria-label="Open ${safe(stage.label)}. ${safe(stage.summary)}" ${viewing?'aria-current="page"':''}><i>${stage.state==='complete'?'&#10003;':String(index+1).padStart(2,'0')}</i><span>${safe(definition.label)}</span></button>`;}).join('')}</nav><span class="flow-next"><b>${next?'Next: '+safe(next.label):'Production complete'}</b><small>${safe(next?.summary||'Every milestone is complete.')}</small></span>`;
  host.querySelectorAll('[data-flow-stage]').forEach(button=>button.onclick=()=>openProductionStage(project.id,button.dataset.flowStage));
}
...
document.querySelector('#render-animatic').insertAdjacentHTML('beforebegin','<select id="master-profile" aria-label="Final video quality"><option value="preview">Small file · up to 720p</option><option value="1080p" selected>Full HD · 1080p</option><option value="4k">Ultra HD · 4K</option></select>');
document.querySelector('#render-animatic').insertAdjacentHTML('afterend','<button id="render-master" type="button">Export final video</button>');
...
async function setupStorageConsole(projectId) {
  ...
  try {
    ...
    document.querySelector('#verify-media-cleanup').onclick=async event=>{event.currentTarget.disabled=true;event.currentTarget.textContent='Verifying…';await api(`/api/projects/${projectId}/media-cleanup/verify`,{method:'POST'});await setupStorageConsole(projectId);};
    ...
    document.querySelector('#create-backup').onclick=async event=>{event.currentTarget.disabled=true;event.currentTarget.textContent='Queueing backup…';await api(`/api/projects/${projectId}/backups`,{method:'POST'});await setupStorageConsole(projectId);};
...
function startMasterExportPolling() {
  stopMasterExportPolling();
  if (!activeMasterExport || !['farm-queued','farm-rendering','assembly-queued','assembling'].includes(activeMasterExport.status)) return;
...
function renderSegmentedExport(job) {
  ...
  const distributed = ['farm-queued','farm-rendering','assembly-queued','assembling'].includes(job.status);
  const stateText = {
    'farm-queued':'Waiting for an available render worker',
    'farm-rendering':'Render farm processing segments automatically',
    'assembly-queued':'Verified segments waiting for an assembly worker',
...
function renderSoundAction(action) { const result=document.querySelector('#sound-producer-result');if(action.status==='proposed'){result.innerHTML=`<div class="crew-action"><div><b>${safe(action.title)}</b><small>${safe(action.summary)}</small><div class="crew-action-buttons"><button id="approve-sound-action" class="primary">Approve performance</button></div></div><span class="status">proposed</span></div>`;document.querySelector('#approve-sound-action').onclick=async()=>{const updated=await api(`/api/crew-actions/${action.id}/approve`,{method:'POST'});renderSoundAction(updated);};}else if(['queued','running'].includes(action.status)){result.innerHTML=`<div class="render-progress">Sound Producer is ${action.status==='running'?'generating and placing the performance':'waiting for an audio worker'}… <small>You can leave this page and track it in Production Activity.</small></div>`;window.setTimeout(()=>watchSoundAction(action.id),3000);}else if(action.status==='completed'){result.innerHTML='<div class="sound-ready">Sound Producer completed and placed the performance.</div>';}else{result.innerHTML=`<div class="job-error">${safe(action.error||action.status)}</div>`;} }
async function watchSoundAction(actionId) { if(!activeAudioStudio)return;try{const crew=await api(`/api/projects/${activeAudioStudio.project_id}/crew`),action=crew.actions.find(item=>item.id===actionId);if(!action)return;if(action.status==='completed'){await refreshAudioAndSelect(activeAudioCueId);return;}renderSoundAction(action);}catch(error){const result=document.querySelector('#sound-producer-result');if(result)result.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;} }
...
let integrationSettings=null,aiRoutingSettings=null,computeSettings=null,storageSettings=null,professionalProfile=null,teamSettings=null,operationalSettings=null,accountBilling=null,settingsView='experience';
...
async function openSettings() {
  if(window.kizunaExperience?.getMode()==='guided')settingsView='experience';
  openWorkspace(settingsDialog);
  const host=document.querySelector('#integration-settings');host.innerHTML=window.kizunaExperience.settingsMarkup()+'<div class="settings-loading">Loading studio connections...</div>';window.kizunaExperience.wire(host);
  try{[integrationSettings,aiRoutingSettings,computeSettings,storageSettings,professionalProfile,teamSettings,operationalSettings]=await Promise.all([api('/api/settings/integrations'),api('/api/settings/ai-routing'),api('/api/settings/compute'),api('/api/settings/storage'),api('/api/settings/creator-profile'),api('/api/settings/team'),api('/api/settings/operations')]);renderIntegrationSettings();}catch(error){host.querySelector('.settings-loading')?.remove();host.insertAdjacentHTML('beforeend',`<div class="job-error">Some connected studio controls could not load: ${safe(error.message)}</div>`);}
}

document.addEventListener('kizuna:workspace-depth',event=>{if(event.detail?.mode==='guided'&&settingsDialog.hasAttribute('open')&&integrationSettings){settingsView='experience';renderIntegrationSettings();}});

function integrationModeLabel(mode) {
  return {api:'Connect by API',handoff:'File handoff',disabled:'Not in use'}[mode]||mode;
}

window.reloadOperationalSettings=async function(){operationalSettings=await api('/api/settings/operations');renderIntegrationSettings();document.querySelector('[data-settings-view="operations"]')?.click();};
...
function renderIntegrationSettings() {
  ...
  host.insertAdjacentHTML('afterbegin',window.renderOperationalSettings?.(operationalSettings)||'');organizeSettingsViews(host);wireComputeControl();window.wireOperationalSettings?.();host.querySelectorAll('[data-ai-route]').forEach(select=>select.onchange=()=>saveAIRoute(select.dataset.aiRoute));host.querySelectorAll('[data-route-model]').forEach(input=>input.onchange=()=>saveAIRoute(input.dataset.routeModel));host.querySelectorAll('[data-integration-form]').forEach(form=>form.onsubmit=event=>saveIntegration(event,form.dataset.integrationForm));host.querySelectorAll('[data-delete-integration]').forEach(button=>button.onclick=()=>deleteIntegration(button.dataset.deleteIntegration));document.querySelector('#custom-integration-form').onsubmit=addCustomIntegration;
}
...
function pendingInvitationMarkup(invite){
  const expired=invite.status==='expired',expires=new Date(invite.expires_at).toLocaleDateString(),access=invite.project_access.map(item=>`${safe(item.project_title)} (${safe(item.role)})`).join(', ');
  return `<article class="${expired?'expired':''}"><span><b>${safe(invite.display_name||invite.email)}</b><small>${safe(invite.email)} · ${access}</small><em>${expired?'Expired':'Expires'} ${safe(expires)}</em></span><div><button type="button" data-resend-invite="${invite.id}">${expired?'Renew link':'Send again'}</button><button type="button" data-revoke-invite="${invite.id}">Revoke</button></div></article>`;
}

function renderTeamSettings(){
  if(!teamSettings)return'';
  if(teamSettings.local_mode)return `<section class="team-settings"><header><div><p class="eyebrow">STUDIO TEAM</p><h3>Collaboration is ready when sign-in is enabled</h3><p>This local studio keeps everything on this computer and does not require accounts. Team invitations and production roles become available in the hosted, signed-in studio.</p></div><span>Local mode</span></header><div class="compute-empty"><b>Your local productions are safe and available.</b><span>${teamSettings.projects.length} production${teamSettings.projects.length===1?'':'s'} can be moved into a hosted team workspace when you are ready.</span></div></section>`;
  ...
  const pending=teamSettings.invitations.map(pendingInvitationMarkup).join('')||'<div class="compute-empty">No pending invitations.</div>';
  const owned=teamSettings.projects.filter(project=>project.my_role==='owner');
  return `<section class="team-settings"><header><div><p class="eyebrow">STUDIO TEAM</p><h3>Invite people into specific productions</h3><p>Owners can direct access. Editors can create and revise. Viewers can review but cannot change production data.</p></div><span>${teamSettings.users.length} account${teamSettings.users.length===1?'':'s'}</span></header><div class="team-project-grid">${projects}</div><div class="team-invite-grid"><form id="team-invite-form"><h3>Invite a collaborator</h3><label>Name<input name="display_name" placeholder="Optional display name"></label><label>Email<input name="email" type="email" required></label><fieldset><legend>Production access</legend>${owned.map(project=>`<div><label><input type="checkbox" name="project_id" value="${project.id}"> ${safe(project.title)}</label><select data-invite-role="${project.id}"><option value="editor">Editor</option><option value="viewer">Viewer</option><option value="owner">Owner</option></select></div>`).join('')||'<small>You need Owner access to invite collaborators.</small>'}</fieldset><button class="primary" type="submit" ${owned.length?'':'disabled'}>Create invitation</button><div id="team-invite-result"></div></form><section><h3>Pending invitations</h3><div id="team-invite-action-result"></div><div class="pending-invites">${pending}</div></section></div></section>`;
}

function organizeSettingsViews(host){
  host.insertAdjacentHTML('afterbegin',window.kizunaExperience.settingsMarkup()+renderTeamSettings()+renderStorageSettings()+renderProfessionalProfile());const experience=document.createElement('div'),compute=document.createElement('div'),team=document.createElement('div'),storage=document.createElement('div'),connections=document.createElement('div'),profile=document.createElement('div'),tabs=document.createElement('nav');experience.className=compute.className=team.className=storage.className=connections.className=profile.className='settings-pane';experience.dataset.settingsPane='experience';compute.dataset.settingsPane='compute';team.dataset.settingsPane='team';storage.dataset.settingsPane='storage';connections.dataset.settingsPane='connections';profile.dataset.settingsPane='profile';tabs.className='settings-tabs';tabs.setAttribute('aria-label','Studio settings');tabs.innerHTML='<button type="button" data-settings-view="experience">Workspace</button><button type="button" data-settings-view="connections">AI & tools</button><button type="button" data-settings-view="compute">Computers & costs</button><button type="button" data-settings-view="storage">Storage & backups</button><button type="button" data-settings-view="team">Team & access</button><button type="button" data-settings-view="profile">Identity & rights</button>';
  [...host.children].forEach(child=>(child.matches('.experience-settings')?experience:child.matches('.compute-control,.spend-monitor')?compute:child.matches('.team-settings')?team:child.matches('.storage-settings')?storage:child.matches('.professional-profile')?profile:connections).appendChild(child));host.replaceChildren(tabs,experience,connections,compute,storage,team,profile);const panes={experience,connections,compute,storage,team,profile},show=view=>{settingsView=view;Object.entries(panes).forEach(([key,pane])=>pane.hidden=view!==key);tabs.querySelectorAll('button').forEach(button=>{const active=button.dataset.settingsView===view;button.classList.toggle('active',active);button.setAttribute('aria-current',active?'page':'false');});};tabs.querySelectorAll('button').forEach(button=>button.onclick=()=>show(button.dataset.settingsView));window.kizunaExperience.wire(host);host.querySelector('#test-s3-storage')?.addEventListener('click',testS3Storage);host.querySelector('#professional-profile-form')?.addEventListener('submit',saveProfessionalProfile);host.querySelector('#professional-work-form')?.addEventListener('submit',saveProfessionalWorkClaim);wireTeamSettings();show(settingsView);
}

function wireTeamSettings(){document.querySelector('#team-invite-form')?.addEventListener('submit',createTeamInvitation);document.querySelectorAll('[data-team-member-role]').forEach(select=>select.onchange=()=>changeTeamMembership(Number(select.dataset.teamProject),Number(select.dataset.teamMemberRole),select.value));document.querySelectorAll('[data-team-add]').forEach(form=>form.onsubmit=event=>addExistingTeamMember(event,Number(form.dataset.teamAdd)));wirePendingInvitations();}
async function refreshTeamSettings(){teamSettings=await api('/api/settings/team');renderIntegrationSettings();settingsView='team';}
async function createTeamInvitation(event){event.preventDefault();const form=event.currentTarget,project_access=[...form.querySelectorAll('[name="project_id"]:checked')].map(input=>({project_id:Number(input.value),role:form.querySelector(`[data-invite-role="${input.value}"]`).value})),result=form.querySelector('#team-invite-result'),submit=form.querySelector('[type="submit"]');if(!project_access.length){result.textContent='Choose at least one production.';return;}submit.disabled=true;submit.textContent='Creating invitation…';try{const invite=await api('/api/settings/team/invitations',{method:'POST',body:JSON.stringify({email:form.elements.email.value,display_name:form.elements.display_name.value,project_access})}),delivery=invite.email_delivery==='queued'?`<div class="invite-delivery-state sent"><b>Email queued</b><span>Kizuna is sending this invitation to ${safe(invite.email)}.</span></div>`:`<div class="invite-delivery-state manual"><b>Send the link manually</b><span>Studio email is not configured, so copy the secure invitation below.</span></div>`;result.innerHTML=`${delivery}<label>Invitation link<input readonly value="${safe(invite.acceptance_url)}"></label><button type="button" id="copy-team-invite">Copy invitation link</button>`;document.querySelector('#copy-team-invite').onclick=async event=>{await navigator.clipboard.writeText(invite.acceptance_url);event.currentTarget.textContent='Copied';};teamSettings=await api('/api/settings/team');renderPendingInvites();form.reset();}finally{submit.disabled=false;submit.textContent='Create invitation';}}
function renderPendingInvites(){const host=document.querySelector('.pending-invites');if(!host)return;host.innerHTML=teamSettings.invitations.map(pendingInvitationMarkup).join('')||'<div class="compute-empty">No pending invitations.</div>';wirePendingInvitations(host);}
function wirePendingInvitations(host=document){host.querySelectorAll('[data-resend-invite]').forEach(button=>button.onclick=()=>resendTeamInvitation(Number(button.dataset.resendInvite),button));host.querySelectorAll('[data-revoke-invite]').forEach(button=>button.onclick=()=>revokeTeamInvitation(Number(button.dataset.revokeInvite)));}
async function resendTeamInvitation(invitationId,button){const result=document.querySelector('#team-invite-action-result');button.disabled=true;button.textContent='Preparing…';try{const invite=await api(`/api/settings/team/invitations/${invitationId}/resend`,{method:'POST'}),delivery=invite.email_delivery==='queued'?'A new email is on its way.':'Email is not configured, so copy this link manually.';result.innerHTML=`<div class="invite-delivery-state ${invite.email_delivery==='queued'?'sent':'manual'}"><b>${invite.email_delivery==='queued'?'Invitation renewed':'New invitation link ready'}</b><span>${safe(delivery)}</span></div><label>New invitation link<input readonly value="${safe(invite.acceptance_url)}"></label><button type="button" data-copy-renewed-invite>Copy invitation link</button>`;result.querySelector('[data-copy-renewed-invite]').onclick=async event=>{await navigator.clipboard.writeText(invite.acceptance_url);event.currentTarget.textContent='Copied';};teamSettings=await api('/api/settings/team');renderPendingInvites();}catch(error){result.textContent=error.message;button.disabled=false;button.textContent='Try again';}}
...
function assistantScreenContext() {
  const openDialog=workspaceDialogs.find(dialog=>dialog.hasAttribute('open')),root=openDialog||document.querySelector('#dashboard-home'),heading=root?.querySelector('h2,h1')?.textContent?.trim()||'Productions',selection=root?.querySelector('.active h3,.active b,.timeline-clip.active b,.cue-item.active b,.character-pill.active b')?.textContent?.trim()||heading;return{heading,selection,workspace:assistantPage(),guidance_mode:window.kizunaExperience?.getMode()||'guided'};
}
...
async function watchCompositeRender(result) { const host=document.querySelector('#composite-result');try{const detail=await api(`/api/jobs/${result.durable_job_id}`),job=detail.job;if(job.status==='completed'){if(activeComposition?.id===result.composition_id)await refreshComposition();return;}if(['failed','cancelled'].includes(job.status)){host.innerHTML=`<div class="job-error">${safe(job.error||`Composite render ${job.status}`)}</div>`;return;}host.innerHTML=`<div class="render-progress">Rendering still composite… ${job.progress_percent}% <small>You can leave this page and track it in Production Activity.</small></div>`;window.setTimeout(()=>watchCompositeRender(result),3000);}catch(error){host.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;} }
document.querySelector('#render-composition').onclick = async () => { if(!activeComposition)return;const button=document.querySelector('#render-composition');button.disabled=true;button.textContent='Queuing preview…';document.querySelector('#composite-result').innerHTML='<div class="render-progress">Sending the frozen layer stack to an available render worker…</div>';try{const result=await api(`/api/compositions/${activeComposition.id}/render`,{method:'POST'});if(result.status==='completed')await refreshComposition();else if(result.status==='failed')document.querySelector('#composite-result').innerHTML=`<div class="job-error">${safe(result.error)}</div>`;else watchCompositeRender(result);}finally{button.disabled=false;button.textContent='Render preview';} };
async function watchMotionRender(result) { const host=document.querySelector('#composite-result');try{const detail=await api(`/api/jobs/${result.durable_job_id}`),job=detail.job;if(job.status==='completed'){if(activeComposition?.id===result.composition_id)await refreshComposition();return;}if(['failed','cancelled'].includes(job.status)){host.innerHTML=`<div class="job-error">${safe(job.error||`Motion render ${job.status}`)}</div>`;return;}host.innerHTML=`<div class="render-progress">Rendering motion preview… ${job.progress_percent}% <small>You can leave this page and track it in Production Activity.</small></div>`;window.setTimeout(()=>watchMotionRender(result),3000);}catch(error){host.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;} }
document.querySelector('#render-motion').onclick = async () => { if(!activeComposition)return;const button=document.querySelector('#render-motion');button.disabled=true;button.textContent='Queuing motion…';document.querySelector('#composite-result').innerHTML='<div class="render-progress">Sending this shot to an available render worker…</div>';try{const result=await api(`/api/compositions/${activeComposition.id}/render-video`,{method:'POST',body:JSON.stringify({quality:'proxy'})});if(result.status==='completed')await refreshComposition();else if(result.status==='failed')document.querySelector('#composite-result').innerHTML=`<div class="job-error">${safe(result.error)}</div>`;else watchMotionRender(result);}finally{button.disabled=false;button.textContent='Render motion preview';} };
...
function showTimelineRender(result) { const settings=result.render_settings||{},host=document.querySelector('#animatic-result');if(settings.kind==='production_master'){host.innerHTML=`<video controls src="${safe(result.uri)}"></video><div class="master-manifest"><b>${safe((settings.profile||'preview').toUpperCase())} FINAL VIDEO</b><span>${settings.width} × ${settings.height} · ${settings.fps} fps</span><span>${settings.motion_clips||0} motion clip${settings.motion_clips===1?'':'s'} · ${settings.fallback_clips||0} fallback clip${settings.fallback_clips===1?'':'s'} · ${settings.audio_cues||0} audio cue${settings.audio_cues===1?'':'s'}</span><a href="${safe(result.uri)}" download>Download final video</a></div>`;}else{host.innerHTML=`<video controls src="${safe(result.uri)}"></video><p><a href="${safe(result.uri)}" download>Download review MP4</a></p>`;} }
async function watchTimelineRender(result) { const host=document.querySelector('#animatic-result');try{const detail=await api(`/api/jobs/${result.durable_job_id}`),job=detail.job;if(job.status==='completed'){const finished={...result,status:'completed',uri:job.result.uri,render_settings:job.result.render_settings};await loadTimeline(activeTimeline.project_id);showTimelineRender(finished);return;}if(['failed','cancelled'].includes(job.status)){host.innerHTML=`<div class="job-error">${safe(job.error||`Timeline render ${job.status}`)}</div>`;return;}host.innerHTML=`<div class="render-progress">${result.render_settings.kind==='production_master'?'Exporting final video':'Rendering review copy'}… ${job.progress_percent}% <small>You can leave this page and track it in Production Activity.</small></div>`;window.setTimeout(()=>watchTimelineRender(result),3000);}catch(error){host.innerHTML=`<div class="job-error">${safe(error.message)}</div>`;} }
document.querySelector('#render-animatic').onclick = async () => { if(!activeTimeline)return;const button=document.querySelector('#render-animatic');button.disabled=true;button.textContent='Queuing proxy…';document.querySelector('#animatic-result').innerHTML='<div class="render-progress">Sending the edit to an available render worker…</div>';try{const result=await api(`/api/timelines/${activeTimeline.id}/render`,{method:'POST'});if(result.status==='completed'){await loadTimeline(activeTimeline.project_id);showTimelineRender(result);}else if(result.status==='failed')document.querySelector('#animatic-result').innerHTML=`<div class="job-error">${safe(result.error)}</div>`;else watchTimelineRender(result);}catch(error){document.querySelector('#animatic-result').innerHTML=`<div class="job-error">${safe(error.message)}</div>`;}finally{button.disabled=false;button.textContent='Render proxy animatic';} };
document.querySelector('#render-master').onclick = async () => { if(!activeTimeline)return;const button=document.querySelector('#render-master'),profile=document.querySelector('#master-profile').value;button.disabled=true;button.textContent='Preparing video…';document.querySelector('#animatic-result').innerHTML=`<div class="render-progress">Preparing the timeline for a ${safe(profile)} export…</div>`;try{const result=await api(`/api/timelines/${activeTimeline.id}/render-master`,{method:'POST',body:JSON.stringify({profile})});if(result.status==='completed'){await loadTimeline(activeTimeline.project_id);showTimelineRender(result);}else if(result.status==='failed')document.querySelector('#animatic-result').innerHTML=`<div class="job-error">${safe(result.error)}</div>`;else watchTimelineRender(result);}catch(error){document.querySelector('#animatic-result').innerHTML=`<div class="job-error">${safe(error.message)}</div>`;}finally{button.disabled=false;button.textContent='Export final video';} };
