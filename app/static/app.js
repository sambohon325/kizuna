...
function renderConnectedStoryMap(result,brief) {
  ...
  result.insertAdjacentHTML('afterbegin',`<section class="story-flow-map connected-story-map"><header><div><span>STORY MAP</span><b>See how the outline connects to production</b></div><small>${covered}/${map.length} beats camera-ready</small></header><div class="story-map-acts">${groups.map(group=>`<section><span>${group.label}</span><div>${group.items.map(item=>`<button type="button" class="${item.status}" data-story-node="${safe(item.beat.position)}"><i>${safe(item.beat.position)}</i><b>${safe(item.beat.name)}</b><small>${item.status==='covered'?'Camera-ready':item.scene?'Needs coverage':'Outline only'}</small></button>`).join('')}</div></section>`).join('')}</div><div id="story-map-detail" class="story-map-detail"></div></section>`);
  ...
  result.querySelectorAll('[data-story-node]').forEach(button=>button.onclick=()=>selectNode(button.dataset.storyNode));selectNode(initial?.beat.position);
}
