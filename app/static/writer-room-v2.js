...
function refreshWriterRoomV2(){
  ...
  const craft=project?.style_profile?.craft||{},traditions=craft.tradition_ids||[],craftLine=traditions.length?`<small>Craft Compass: ${safe(traditions.join(' - '))}. Treat these as questions, not formulas.</small>`:'<small>The Craft Compass is not set yet. Style Lab can connect story, image, motion, and sound around shared traditions.</small>';const guide=document.querySelector('#writer-v2-guide');if(guide)guide.innerHTML=`<b>${writerV2Stage[0].toUpperCase()+writerV2Stage.slice(1)} focus</b><p>${guidance.join(' · ')}</p>${craftLine}`;
