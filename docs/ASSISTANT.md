...
The current local assistant provides deterministic workflow-aware guidance without requiring an external account. The provider router will allow the same project and screen context to be handled by configured hosted, local, or homebrew AI engines while keeping the interface unchanged.
