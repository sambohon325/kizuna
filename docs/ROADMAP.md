...
- retain the implemented Operations readiness dashboard, backup archive verification, structured logs, core-service heartbeats, scanner health, in-app operating notices, and recurring non-destructive archive recovery drills, then add external alert delivery and a full-stack rehearsal.
...
3. **Schema migrations and production database verification — implemented:** Alembic owns a complete baseline, safely adopts matching legacy databases without losing productions, rejects unsafe partial schemas, and orders Coolify services behind a dedicated migration job. GitHub Actions runs the full suite on PostgreSQL and SQLite; PostgreSQL is independently checked before and after pytest for current revision, complete migrated tables, JSON round-trip behavior, and rollback safety.
4. **Operational readiness — first four slices implemented:** Docker keeps a lightweight liveness endpoint, while administrators now have database, Redis fallback, writable-storage, capacity, durable-queue, expired-lease, backup-state, web/worker/scheduler heartbeat, compliance-scanner, recovery-drill, and external-delivery diagnostics in Studio Settings. The newest local backup is rehearsed weekly. Operational errors can be delivered through SMTP and signed generic webhooks with severity filtering, cooldown deduplication, retries, safe delivery history, and manual testing. Web requests and worker, scheduler, scanner, jobs, and alert delivery emit structured JSON for Coolify. Next run an isolated full-stack Coolify rehearsal, design guarded project import, and formalize log retention/export before public trials.
