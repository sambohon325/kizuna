...
4. **Operational readiness — first three slices implemented:** Docker keeps a lightweight liveness endpoint, while administrators now have database, Redis fallback, writable-storage, capacity, durable-queue, expired-lease, backup-state, web/worker/scheduler heartbeat, compliance-scanner, and recovery-drill diagnostics in Studio Settings. Warnings and errors include a concrete operating action. The newest local backup is rehearsed weekly: every archived byte is read, the production and media counts are validated, a temporary recovery catalog is rebuilt and reopened, and all rehearsal files are removed. Web requests and worker, scheduler, scanner, and job-lifecycle events are emitted as structured JSON for Coolify. Accounts, invitations, roles, production isolation, password recovery, protected trials, and Stripe entitlement foundations are also implemented. Next add PostgreSQL CI, external alert delivery, an isolated full-stack Coolify rehearsal, and a guarded project-import design before public trials.
