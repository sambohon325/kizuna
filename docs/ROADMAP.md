...
- retain the implemented Operations readiness dashboard, backup archive verification, structured logs, core-service heartbeats, scanner health, and in-app operating notices, then add external alert delivery and recurring restore drills.
...
4. **Operational readiness — first two slices implemented:** Docker keeps a lightweight liveness endpoint, while administrators now have database, Redis fallback, writable-storage, capacity, durable-queue, expired-lease, backup-state, web/worker/scheduler heartbeat, and compliance-scanner diagnostics in Studio Settings. Warnings and errors include a concrete operating action. The latest local backup can be read end-to-end to verify its checksum, ZIP entries, manifest version, and production identity without overwriting production data. Web requests and worker, scheduler, scanner, and job-lifecycle events are emitted as structured JSON for Coolify. Accounts, invitations, roles, production isolation, password recovery, protected trials, and Stripe entitlement foundations are also implemented. Next add PostgreSQL CI, external alert delivery, automated recurring restore drills, and a final Coolify disaster-recovery runbook before public trials.
