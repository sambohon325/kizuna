...
2. **Durable job foundation — in progress:** the shared database job envelope, Redis wakeups, inline development fallback, worker leases, retry/cancellation controls, event history, proxy executor, media-transfer integration, non-destructive storage safety audits, and manual/scheduled production backups are implemented. The creator-facing Activity workspace exposes progress, failures, cancellation, retry, and job history. Next migrate crew and render jobs.
